...
SELECT COUNT(*) total, SUM(DECODE(TO_CHAR(hire_date, 'YYYY'),2009,1,0))"2009", SUM(DECODE(TO_CHAR(hire_date, 'YYYY'),2010,1,0))"2010", SUM(DECODE(TO_CHAR(hire_date, 'YYYY'),2011,1,0))"2011", SUM(DECODE(TO_CHAR(hire_date, 'YYYY'),2012,1,0))"2012"
FROM employees;

SELECT job_id "Job", SUM(DECODE(department_id , 20, salary)) "Dept 20", SUM(DECODE(department_id , 50, salary)) "Dept 50", SUM(DECODE(department_id , 80, salary)) "Dept 80", SUM(DECODE(department_id , 90, salary)) "Dept 90", SUM(salary) "Total"
FROM employees GROUP BY job_id;
